<template>
  <div class="show-box" :class="{disabled: disabled, active: isShowList}" :style="style_Container">
    <!-- 输入框，仅在可输入模式下使用 -->
    <input v-if="showInput" class="input" placeholder-style="color: #bbb;" type="text" v-model="selectText"
      :placeholder="placeholder" @focus="onFocus" @blur="onBlur" @input="onInput" @confirm="$emit('confirm', $event)">
    <!-- 显示框 -->
    ...
    <div class="list-container" @click.stop="onListClick" :style="'top:' + listTop__ + 'px;'" v-show="isShowList">
      ...
      <scroll-view class="list" style="background-color: #fff;" :style="'max-height: ' + listBoxHeight__ +'em;'"
        scroll-y=true scroll-x=true>
        <div class="item" @click="onClickItem(index, item.value)" v-for="(item, index) in innerList" :key="index"
  ...
  function getNodeInfo({
    ...
    stepRunFunc((next, currNum) => {
      selectorQuery.exec(([data]) => { // 开始查询页面中的节点
        ...
        data || totalAttemptNum <= currNum ? typeof callback === 'function' && callback.call(thisObj, data) :
